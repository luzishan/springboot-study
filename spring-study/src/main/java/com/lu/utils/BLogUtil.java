...
public class BLogUtil {
    ...
    public void methodStart(JoinPoint joinPoint) {
        System.out.println("BLogUtil"+joinPoint.getSignature().getName() + "方法执行前,参数分别是：" + Arrays.asList(joinPoint.getArgs()));
    }

    @After("logPointCut()")
    public void methodEnd(JoinPoint joinPoint) {
        System.out.println("BLogUtil"+joinPoint.getSignature().getName() + "方法执行后");
    }

    @AfterThrowing("logPointCut()")
    public void methodThrow(JoinPoint joinPoint) {
        System.out.println("BLogUtil"+joinPoint.getSignature().getName() + "方法执行抛出异常");
    }

    @AfterReturning(value = "logPointCut()", returning = "result")
    public void methodReturn(JoinPoint joinPoint, Object result) {
        System.out.println("BLogUtil"+joinPoint.getSignature().getName() + "方法执行返回结果是：" + result);
    }
